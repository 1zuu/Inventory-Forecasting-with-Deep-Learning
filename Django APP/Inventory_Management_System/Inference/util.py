...
from Inference.variables import *
...
def normalize_data(train_data, test_data):
    ...
    minmax_scaler.fit(Y)
    Y = minmax_scaler.transform(Y)
    
    if not os.path.exists(minmax_scaler_weights):
        joblib.dump(minmax_scaler, minmax_scaler_weights)
    else:
        minmax_scaler = joblib.load(minmax_scaler_weights)
...
def get_item_id(food):
    food = food.strip().lower()
    df = pd.read_csv(food_csv_path)
    df['Foods'] = df['Foods'].str.strip()
    df['Foods'] = df['Foods'].str.lower()
    df = df.loc[df['Foods'] == food]
...
def process_sample_input(sample_input):
    ...
    food = str(sample_input['food'])

    item = get_item_id(food)
    ...
    return int(df_input['id'].values)

def get_inference_data(Xtest, sample_input):
    idx = process_sample_input(sample_input)
    return Xtest[idx]