...
from variables import sample_input
...
def insert():
        ...
        try:
            ...
            sales = make_predictions(inputs)
            print('--------------------------------------------------------------------------------------')
            print('{} {}s'.format(sales, prodname))
            print('--------------------------------------------------------------------------------------')
               ...
               if request.method == 'POST':
                    ...
                    if request.method == 'POST':
                         ...
                         if mycursor.fetchone()[0]:
                            ...
                            mycursor.execute("INSERT INTO flowerinventory(product_id,product_name, price ,quantity,date,store) Values(%s,%s,%s,%s,%s,%s)", (prodid, prodname, prodprice, prodquant, proddate, prodstore))
                            mydb.commit()
                            mycursor.close()
                            mydb.close()
                            flash(prodid + ',' + prodname + ', ' + prodprice + ', ' + prodquant + ',' + proddate + ',' + prodstore + ', Successfully saved!')
                            return render_template('index.html')
                    else:
                        return render_template('index.html')
        except Exception as e:
